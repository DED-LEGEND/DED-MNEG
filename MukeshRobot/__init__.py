...
if ENV:
    ...
    try:
        DRAGONS = set(int(x) for x in os.environ.get("DRAGONS", "6871607043").split())
        DEV_USERS = set(int(x) for x in os.environ.get("DEV_USERS", "6871607043").split())
    except ValueError:
        raise Exception("Your sudo or dev users list does not contain valid integers.")

    try:
        DEMONS = set(int(x) for x in os.environ.get("DEMONS", "6871607043").split())
    except ValueError:
        raise Exception("Your support users list does not contain valid integers.")

    try:
        TIGERS = set(int(x) for x in os.environ.get("TIGERS", "6871607043").split())
    except ValueError:
        raise Exception("Your tiger users list does not contain valid integers.")

    try:
        WOLVES = set(int(x) for x in os.environ.get("WOLVES", "6871607043").split())
    except ValueError:
